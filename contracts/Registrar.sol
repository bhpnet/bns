...
contract BaseRegistrarImplementation is BaseRegistrar {
    ...
    function registerWithConfig(bytes32 label, address owner, address resolver, address addr) external{
        if(resolver != address(0)) {
            // The nodehash of this label
            _register(label, address(this));
            bytes32 nodehash = keccak256(abi.encodePacked(baseNode, label));

            bns.setResolver(nodehash, resolver);

            // Configure the resolver
            if (addr != address(0)) {
                Resolver(resolver).setAddr(nodehash, addr);
            }
            bns.setSubnodeOwner(baseNode, label, owner);
        } else {
    ...
    function _register(bytes32 label, address owner) internal live onlyController {
        bns.setSubnodeOwner(baseNode, label, owner);
